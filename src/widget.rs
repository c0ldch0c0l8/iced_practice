...
use iced::{Background, Color, Column, Length, Sandbox, Size, Slider, Space, slider};
...
impl Sandbox for WidgetApp {
    ...
    fn view(&mut self) -> iced::Element<'_, Self::Message> {
        Column::new()
        .push( Circle { radius: self.radius } )
        .push( Space::with_height(Length::Fill) )
        .push(
            Slider::new(&mut self.slider_state, 0..=256, self.radius, Message::RadiusChanged)
        )
        .into()
    }
